...
class VGG16(nn.Module):
    ...
    def forward_gpu(self, x, locations, p_height, p_width, beta=None):
        ...
        p_height, p_width = inc_convolution(self.image.data, x, self.conv1_1_op[0].weight.data,
                                            self.conv1_1_op[0].bias.data,
        ...
        p_height, p_width = inc_convolution(self.conv1_1.data, x, self.conv1_2_op[0].weight.data,
                                            self.conv1_2_op[0].bias.data,
        ...
        p_height, p_width = inc_convolution(self.pool1.data, x, self.conv2_1_op[0].weight.data,
                                            self.conv2_1_op[0].bias.data,
                                            out.data, locations.data, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv2_1.data, x, self.conv2_2_op[0].weight.data,
                                            self.conv2_2_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.pool2.data, x, self.conv3_1_op[0].weight.data,
                                            self.conv3_1_op[0].bias.data,
                                            out.data, locations.data, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv3_1.data, x, self.conv3_2_op[0].weight.data,
                                            self.conv3_2_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv3_2.data, x, self.conv3_3_op[0].weight.data,
                                            self.conv3_3_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.pool3.data, x, self.conv4_1_op[0].weight.data,
                                            self.conv4_1_op[0].bias.data,
                                            out.data, locations.data, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv4_1.data, x, self.conv4_2_op[0].weight.data,
                                            self.conv4_2_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv4_2.data, x, self.conv4_3_op[0].weight.data,
                                            self.conv4_3_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.pool4.data, x, self.conv5_1_op[0].weight.data,
                                            self.conv5_1_op[0].bias.data,
                                            out.data, locations.data, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv5_1.data, x, self.conv5_2_op[0].weight.data,
                                            self.conv5_2_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
        ...
        p_height, p_width = inc_convolution(self.conv5_2.data, x, self.conv5_3_op[0].weight.data,
                                            self.conv5_3_op[0].bias.data,
                                            out, locations, 1, 1, 1, 1, p_height, p_width, beta)
        x = F.relu(out)
        if debug: print(locations, p_height, x.shape)
    ...
    def forward_pytorch(self, patches, locations, p_height, p_width, beta=None):
        ...
        for layer, data, c, size, s, p, k in zip(layers, premat_data, C, sizes, S, P, K):
            ...
            out_locations = self.__get_output_locations(in_locations, out_p_size, s, p, k, prev_size, size,
                                                        remove=remove)
            ...
            for i in range(batch_size):
                ...
                temp = data[0, :, :, :].clone()
                temp[:, in_locations[i][0]:in_locations[i][0] + orig_patch_size,
                in_locations[i][1]:in_locations[i][1] + orig_patch_size] = patches[i, :, :, :]

                x[i, :, x0:x1, y0:y1] = temp[:, max(s * out_locations[i][0] - p, 0):max(0, s * out_locations[i][
                    0] - p) + x1 - x0,
                                        max(0, s * out_locations[i][1] - p):max(0,
                                                                                s * out_locations[i][1] - p) + y1 - y0]
    ...
    def __get_tensor(self, name, batch_size, channels, p_height, p_width, k_size, stride, in_size, out_size,
        ...
        if name in self.tensor_cache:
            ...
            tensor = torch.FloatTensor(batch_size, channels,
                                       *self.__get_output_shape(p_height, p_width, k_size, stride, in_size, out_size,
                                                                truncate))
            if self.gpu:
                tensor = tensor.cuda()
