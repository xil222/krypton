...
for iteration in range(4):
    ...
    for i in range(5):
        ...
        patch_location_tensor = Variable(torch.from_numpy(
                np.array([(in_size - p_size) // 2, (in_size - p_size) // 2] * batch_size,
                         dtype=np.int32)).cuda())
        inc_convolution(in_tensor.data, weights.data, biases.data, out_tensor.data, patch_location_tensor.data, 1, 1, p_size, p_size, 1.0)
        torch.cuda.synchronize()
    torch.cuda.synchronize()
    print('inc conv v' + ': ' + str(time.time() - prev_time))
