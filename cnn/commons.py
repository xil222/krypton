...
class IncMaxPoolFunction(Function):
    ...
    def __init__(self, padding, stride, k_size, p_height, p_width):
...
class IncMaxPoolModule(Module):
    ...
    def __init__(self, in_tensor, out_tensor, padding, stride, k_size):
    ...
    def forward(self, patch_location_tensor, p_height=0, p_width=0):
        ...
        return IncMaxPoolFunction(self.padding, self.stride, self.k_size, p_height, p_width)(self.in_tensor,  self.out_tensor, patch_location_tensor),\
               (out_p_height, out_p_width)


class IncConvFunction(Function):

    def __init__(self, padding, stride, p_height, p_width):
...
class IncConvModule(Module):
    ...
    def __init__(self, in_tensor, weights, biases, out_tensor, padding, stride, k_size):
    ...
    def forward(self, patch_location_tensor, p_height=0, p_width=0):
        ...
        out_p_height = int(math.ceil((p_height+self.k_size-1) * 1.0 / self.stride))
        out_p_width = int(math.ceil((p_width+self.k_size-1) * 1.0 / self.stride))
        return IncConvFunction(self.padding, self.stride, p_height, p_width)(self.in_tensor, self.weights, self.biases, self.out_tensor,
