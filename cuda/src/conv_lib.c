  ...
  int stride_x, int stride_y, int p_height, int p_width, float beta)
{
    ...
    int out_width = (in_width - k_size_x + 2*padding_x)/stride_x + 1;
    int out_height = (in_height - k_size_y + 2*padding_y)/stride_y + 1;
 
    int temp_p_height = min((int)ceil((p_height+k_size_y-1)*1.0/stride_y), out_height);
    int temp_p_width = min((int)ceil((p_width+k_size_x-1)*1.0/stride_x), out_width);
    ...
    int in_p_height = k_size_y + (out_p_height-1)*stride_y;
    int in_p_width = k_size_x + (out_p_width-1)*stride_x;
    ...
    extract_input_volume_gpu(ptr_premat_tensor, ptr_in_tensor, temp_in_tensor, ptr_location, batch, in_channels,
     in_height, in_width, stride_x, stride_y, padding_x, padding_y, p_height, p_width, in_p_height, in_p_width,
     k_size_x, k_size_y, patch_growing);

    update_output_locations_gpu(ptr_location, batch, in_height, in_width, padding_x, padding_y, stride_x, stride_y, k_size_x,
     k_size_y, in_p_height, in_p_width, patch_growing);
 ...
 int padding_x, int padding_y, int stride_x, int stride_y, int k_size_x, int k_size_y, int p_height, int p_width, float beta)
{
    ...
    inc_max_pool_gpu(ptr_premat_tensor, ptr_in_tensor, ptr_out_tensor, ptr_location, in_height, in_width, p_height, p_width,
     in_channels, batch, padding_x, padding_y, stride_x, stride_y, k_size_x, k_size_y, out_p_height, out_p_width, patch_growing);
