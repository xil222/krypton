...
\red{Methods used for explain CNN predictions can be broadly divided into two categories, perturbation based and gradient based methods.
Perturbation based methods observe the output of the model by modifying regions of the input image.
...
\red{Gradient-based methods generate a sensitivity heat map by computing the partial derivatives of model output with respect to every input pixel using the backpropagation method.
...
We evaluated the runtime and visual quality of the generated heat maps for integrated gradients and OBE methods on three representative images from our datasets.
Due to space constraints we present the results in Appendix Figure \ref{fig:igd}} 
