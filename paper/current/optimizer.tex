...
\begin{table}[t]
  ...
  \scalebox{0.8}{\begin{tabular}{p{2cm}p{7.5cm}}
    ...
    \midrule \midrule
    $^lx^\mathcal{I}_\mathcal{P},{}^ly^\mathcal{I}_\mathcal{P}$ & Starting coordinates of the input patch for the $l^{th}$ layer\\
    \midrule
    $^lx^\mathcal{R}_\mathcal{P},{}^ly^\mathcal{R}_\mathcal{P}$ & Starting coordinates of the patch that needs to be read in for the $l^{th}$ layer transformation\\
    \midrule
    $^lx^\mathcal{O}_\mathcal{P},{}^ly^\mathcal{O}_\mathcal{P}$ & Starting coordinates of the output patch for the $l^{th}$ layer\\
    \midrule
    $^lH^\mathcal{I}_\mathcal{P},{}^lW^\mathcal{I}_\mathcal{P}$ & Height and width of the input patch for the $l^{th}$ layer\\
   	\midrule
   	$^lH^\mathcal{R}_\mathcal{P},{}^lW^\mathcal{R}_\mathcal{P}$ & Height and width of the patch that needs to be read in for the $l^{th}$ layer transformation\\
   	\midrule
   	$^lH^\mathcal{O}_\mathcal{P},{}^lW^\mathcal{O}_\mathcal{P}$ & Height and width of the output patch for the $l^{th}$ layer\\
    \midrule
...
Use of padding effectively shifts the coordinate system and therefore $^lP_x$ is added to correct it.
Due to the overlapping nature of filter kernels the affected span of the updated patch in the input will be increased by $^lW_\mathcal{K}-1$ amount and hence needs to be subtracted from the input coordinates $^lx^\mathcal{I}_\mathcal{P}$ (a filter of size $^lW_\mathcal{K}$ which is placed starting at $^lx^\mathcal{I}_\mathcal{P} - {}^lW_\mathcal{K} + 1$ will see the new change at $^lx^\mathcal{I}_\mathcal{P}$).
...
\eat{%%Yannis: Too verbose. Writing that narrates the equations is not really useful.
...
\begin{algorithm}
    ...
    \For{\texttt{i in [1,...,n]}}\label{alg:line:memcpy_loop}
    	\State $T_1 \gets {}^l\mathcal{I}[:,{}^lx^\mathcal{R}_{\mathcal{P}_i}:{}^lx^\mathcal{R}_{\mathcal{P}_i}+{}^lW^\mathcal{R}_\mathcal{P},{}^ly^\mathcal{R}_{\mathcal{P}_i}:{}^ly^\mathcal{R}_{\mathcal{P}_i}+{}^lH^\mathcal{R}_\mathcal{P}]$ 
    	\State $T_2 \gets T_1 \bm\circ_{({}^{l}x^\mathcal{I}_{\mathcal{P}_i}-{}^{l}x^\mathcal{R}_{\mathcal{P}_i}),({}^ly^\mathcal{I}_{\mathcal{P}_i}-{}^ly^l\mathcal{R}_{\mathcal{P}_i})} {}^l\mathcal{P}_i$
    	\State $R[i,:,:] \gets T_2$
    \EndFor

    \State $[{}^l\mathcal{P}^\mathcal{O}_1,...,{}^l\mathcal{P}^\mathcal{O}_n] \gets T(\mathcal{R})$
    \State \textbf{return} $[{}^l\mathcal{P}^\mathcal{O}_1,...,{}^l\mathcal{P}^\mathcal{O}_n]$,
    \State \hspace*{5mm} $[({}^lx^\mathcal{O}_{\mathcal{P}_1},{}^ly^\mathcal{O}_{\mathcal{P}_1}),...,({}^lx^\mathcal{O}_{\mathcal{P}_n},{}^ly^\mathcal{O}_{\mathcal{P}_n})],$ (${}^lW^\mathcal{O}_\mathcal{P},{}^lH^\mathcal{O}_\mathcal{P}$) 
    \EndProcedure
    \end{algorithmic}
...
With all the geometric mappings defined we now explain the complete incremental inference approach for a single layer.
Algorithm \ref{alg:incinference} presents it formally.
The \textproc{IncrementalInference} procedure takes in the original transformation $^lT$ of the $l^{th}$ layer, pre-materialized input for the layer corresponding to the original image, a batch of updated patches which are 3D volumes of activation values and their geometric properties as input.
...
First \textproc{IncrementalInference} calculates the geometric properties of the output and the read-in patches.
...
In a CNN which has multiple such layers chained together, the outputs of the \textproc{IncrementalInference} procedure are fed as input again for the incremental inference of the next layer along with the pre-materialized input from the original image.
However, at a boundary of local context transformation and a global context transformation, such as in Convolution $\rightarrow$ Fully-Connected or Pool $\rightarrow$ Fully-Connected, full updated output has to be created \eat{as per \textproc{IncrementalToFullProjection} procedure} instead of propagating only the updated patches.
