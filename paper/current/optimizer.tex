...
In this section we focus on transformations that operate at the granularity of a local spatial context (i.e. Convolution and Pooling) as other types either has no redundancies (global context transformations) or is trivial to make incremental aware (point transformations).
...
Use of padding effectively shifts the coordinate system and therefore $P_x$ is added to correct it.
Due to the overlapping nature of filter kernels the affected span of the updated patch in the input will be increased by $W_\mathcal{K}-1$ amount and hence needs to be subtracted from the input coordinates $x^\mathcal{I}_\mathcal{P}$ (a filter of size $W_\mathcal{K}$ which is placed starting at $x^\mathcal{I}_\mathcal{P} - W_\mathcal{K} + 1$ will see the new change at $x^\mathcal{I}_\mathcal{P}$).
Dividing the above value by the stride values $S_x$ and taking the \texttt{ceil} gives the starting coordinates of the output patch (essentially calculates the number of strides).
Towards the left side edge of the input where the affected span of the input cannot be extended by $W_\mathcal{K}-1$ amount this value will be negative.
Therefore the maximum of zero or the above value should be taken as the final.
Equation \ref{eqn:patchwidth} calculates the width and height of the output patches.
Similar to output coordinate calculation the span of the input patch is increased by $W_\mathcal{K}-1$ amount.
Dividing the above value by the stride value $S_x$ and taking the \texttt{ceil} gives the width of the output patch.
Since the output patch cannot grow beyond the size of the output, minimum of the output width and the calculated value should be taken as the final value.
Once the output patch coordinate and width are calculated it is straight forward to calculate the read-in patch coordinate as per Equation \ref{eqn:xreadcoordinate} and the width as per Equation \ref{eqn:readpatchwidth}.
...
\begin{algorithm}
    ...
    \For{\texttt{i in [1,...,n]}}\label{alg:line:memcpy_loop}
    	...
    	\State $T_2 \gets T_1 \bm\circ_{(x^\mathcal{I}_{\mathcal{P}_i}-x^\mathcal{R}_{\mathcal{P}_i}),(y^\mathcal{I}_{\mathcal{P}_i}-y^\mathcal{R}_{\mathcal{P}_i})} \mathcal{P}_i$
    	\State $R[i,:,:] \gets T_2$
    \EndFor
...
With all the geometric mappings defined we now explain the complete incremental inference approach for a single transformation.
...
In a CNN which has multiple such transformations chained together, the outputs of the \textproc{IncrementalInference} procedure are fed as input again for the incremental inference of the next transformation along with the unchanged pre-materialized input.
...
We analyze the theoretical speedup that can be achieved with \textit{incremental inference} approach when a square occlusion patch is placed on the center\footnote{If the occlusion patch is placed towards to a corner of the input image the theoretical speedup will be slightly higher.
But placing the occlusion patch on the center gives us a worst case estimate.} of the input image. Figure \ref{fig:redundancy_ratio} shows the results.
VGG16 model results in the maximum theoretical speedup and DenseNet121 model has the lowest theoretical speedup. Most CNN architectures results in a theoretical speedup between 2-3.
The theoretical speedup for a CNN with \textit{incremental inference} is determined by the characteristics of it's architecture such as number of layers, the sizes of the filter kernels, and the filter stride values.
...
x^\mathcal{O}_P =&~ \texttt{min}(x^\mathcal{I}_{\mathcal{P}_1},x^\mathcal{I}_{\mathcal{P}_2})\\
...
W^\mathcal{O}_\mathcal{P} =&~ \texttt{max}(x^\mathcal{I}_{\mathcal{P}_1}+W^\mathcal{I}_{\mathcal{P}_1},x^\mathcal{I}_{\mathcal{P}_2}+W^\mathcal{I}_{\mathcal{P}_2}) -\texttt{min}(x^\mathcal{I}_{\mathcal{P}_1},x^\mathcal{I}_{\mathcal{P}_2})
...
\subsection{Approximate: Projective Field Thresholding}
...
The three types of CNN transformations affects the size of the projective field differently.
Point transformations does not change the projective field size while global context transformations increases it to the maximum.
Transformations that operate on a local spatial context increase it gradually.
The amount of increase in a local context transformation is determined the filter size and stride parameters.
...
For a more intuitive understanding on why this would work consider the simplified one dimensional convolution example shown in Figure \ref{fig:pf_truncate} (a).
...
We also analyze the mean square error (MSE) between exact and approximate output of the activation volume produced by the final Conv. or Pool transformation with a black occlusion patch placed on the center of the input image.
...
\subsection{Approximate: Adaptive Drill-Down}\label{sec:ada-drill-down}
...
In \textit{adaptive-drill-down} the final occlusion heatmap will be generated using a two stage process.
At the first stage a low resolution heatmap will be generated by using a larger stride which we call stage one stride $S_1$.
From the heatmap generated at stage one a predefined drill-down fraction $r_{drill-down}$ of regions with highest probability drop for the predicted class is identified.
...
A schematic representation of \textit{adaptive drill-down} is shown in Figure \ref{fig:adaptive-drill-down}.
...
But it has the risk of mis-identifying interesting regions specially when the granularity of those regions are smaller than the occlusion patch size.
...
Figure \ref{fig:r_and_s1} shows conceptually how the speedup would vary with $S_1$ when $r_{drill-down}$ is fixed and vice versa.
...
At operational time, \system~ requires the user to provide the expected level of quality for the heatmaps in terms of a SSIM value.
