...
We integrated all of our optimization techniques with the popular deep learning environment PyTorch to create a tool we call \system~(\red{more details in Appendix \ref{sec:pytorch_integration}}).
...
\vspace{2mm}
\noindent \textbf{Memory Overhead.} \red{We evaluated the memory overhead of IVM approach compared to the full CNN inference method and found that it can enable up to 58\% lower memory overheads (see Appendix \ref{sec:mem_overhead}).}
...
\red{For this first paper on this work, we decided to use OBE as the concrete use case to demonstrate the effectiveness of our IVM framework.
However, the applicability of our optimizations can go beyond just the OBE workload.
Our IVM approach may be applied to optimize a broad range of complex visual recognition and video analytics tasks where the contents of subsequent frames does not change significantly ~\cite{cavigelli2017cbinfer,buckler2018eva}.
On the other hand, one can treat the IVM friendliness of a CNN as a selection criteria for the model selection process when selecting an IVM accelerated CNN architecture for video analytics ~\cite{motamedi2018resource}.
However, these require addressing several open challenges such as batching arbitrary sized update patches to utilize the maximum hardware throughput which we leave for future work.}