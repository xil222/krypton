...
We integrated all of our optimization techniques with the popular deep learning environment PyTorch to create a tool we call \system~(\red{for more details on \system~ integration into PyToch see Appendix \ref{sec:pytorch_integration}}).
...
\vspace{2mm}
\noindent \textbf{Memory Overhead.} \red{We also evaluated the memory overhead of IVM approach compared to the full CNN inference method and found that it can enable up to 58\% lower memory overheads (see Appendix \ref{sec:mem_overhead}).}
