...
\vspace{2mm}
\noindent \textbf{Memory Overhead.} \red{We evaluate the memory overhead of our incremental inference approach compared to the full CNN inference.
We record the peak GPU memory utilization while the CNN models perform inference on image batches of size 128.
We found that incremental inference approach has a significantly lower memory overhead compared to the full inference approach as the size of the intermediate memory buffers required for propagating the changes is small.
Due space constraints the results are shown in Appendix Figure \ref{fig:mem_overhead}.
}
